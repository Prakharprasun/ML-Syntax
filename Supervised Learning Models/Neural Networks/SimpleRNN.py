...
# Build the RNN model
model = Sequential()
model.add(SimpleRNN(64, activation='relu', return_sequences=False, input_shape=(time_step, 1)))
...
plt.title('Stock Price Prediction with RNN (TensorFlow)')
plt.xlabel('Days')
plt.ylabel('Price')
plt.legend()
plt.show()
...
class RNN(nn.Module):
    ...
    def forward(self, x):
        _, hn = self.rnn(x)  # Get the hidden state at the last timestep
        out = self.fc(hn.squeeze(0))  # Final output from the hidden state
        return out
...
plt.plot(y_test, label='True Stock Price')
plt.plot(y_pred, label='Predicted Stock Price')
plt.title('Stock Price Prediction with RNN (PyTorch)')
plt.xlabel('Days')
plt.ylabel('Price')
plt.legend()
plt.show()
